class ThrowableObject extends MovableObject {
  ...
  throw() {
    this.speedY = 15; this.applyGravity();
    this.intervalThrow = setInterval(() => {
      this.bottleOnTheGround();
      if (this.isBroken) this.stopBottleAnimate(); else this.x += 2;
    }, 10);
  }
  ...
  /**
   * Plays splash sound once as soon as the bottle is broken.
   */
  handleSplashSound() {
  ...
  bottleOnTheGround() {
    if (this.y >= 360 && !this.isBroken) {
      this.isBroken = true;
      this.speed = 0; this.speedY = 0; this.acceleration = 0;
